...
<head>
    ...
    <link rel="icon" type="image/svg+xml" href="{{ asset('favicon-tte.svg') }}">
...
<body class="min-h-screen bg-slate-100 text-slate-800">
    <nav class="bg-white shadow-sm ring-1 ring-slate-200">
        <div class="flex w-full gap-2 px-4 py-3 sm:px-6 lg:px-8">
            ...
            <a href="{{ session()->has('admin_id') ? route('admin.dashboard') : route('admin.login.form') }}" class="rounded-lg px-3 py-2 text-sm font-medium {{ request()->routeIs('admin.*') ? 'bg-blue-100 text-blue-700' : 'text-slate-700 hover:bg-slate-100' }}">Admin</a>
        </div>
    </nav>
